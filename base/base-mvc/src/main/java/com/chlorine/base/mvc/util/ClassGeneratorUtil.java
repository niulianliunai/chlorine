...
public class ClassGeneratorUtil {
     ...
     */
    public static void generate(String filePath, String packageName, String baseClassPath) {
        String entityPath = filePath + "/entity/";
        if (!FileUtil.exist(entityPath)) {
            throw new RuntimeException("缺少entity");
        }

        generateQTableConstantClass(filePath,packageName);
        generateJpaQueryConfig(filePath,packageName);

        String repositoryPath = filePath + "/repository/";
        String servicePath = filePath + "/service/";
        String controllerPath = filePath + "/controller/";
    ...
    private static void generateQTableConstantClass(String filePath, String packageName) {
        File entityFolder = new File(filePath + "/entity/");
        ...
        sb.append("}\n");
        File constantPath = new File(filePath+"/constant/");
        if (!constantPath.exists()) {
            constantPath.mkdirs();
        }
        generateFile(new File(filePath + "/constant/QTableConstant.java"), sb.toString());
    }
